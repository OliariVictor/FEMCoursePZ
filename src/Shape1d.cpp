    ...
    for(int i :orders){
        ...
        if (no < 2){
            if (orders[no] == 1) {
                ...
                phi[1] = 0.5*(1 + xi[0]); dphi(0,1) = +0.5;
            } else {std::cout << "Shape functions on corner nodes are strictly linear"; DebugStop();}
        }
        else if (no == 2) {
            if (orders[no] == 2 ) {phi[2] = phi[0]*phi[1]; dphi(0,2) = dphi(0,0)*phi[1] + dphi(0,1)*phi[0]; }
            else { int size = NShapeFunctions(orders); phi.resize(size);dphi.Resize(1,size);}
        } no++;
      }
}
